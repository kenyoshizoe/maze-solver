#include "maze-solver/solver/astar.h"
...
Path Astar::Solve(Maze maze) {
  ...
  auto compare = [](const Node& lhs, const Node& rhs) {
    return lhs.cumulative_cost + lhs.heuristic_cost >
           rhs.cumulative_cost + rhs.heuristic_cost;
  };
  ...
  while (true) {
    ...
    for (int i = 0; i < next_paths.size(); i++) {
      ...
      open.push({next_paths.at(i),
                 n.cumulative_cost + 1 + rotate_cost + acc_cost,
                 CalcHeuristicCost(next_paths.at(i), maze)});
    }
...
int Astar::CalcHeuristicCost(const Path& path, const Maze& maze) const {
  return std::abs(path.GetPosition().x - maze.GetGoal().x) +
         std::abs(path.GetPosition().y - maze.GetGoal().y);
}
}  // namespace maze_solver
